...
# 处理好之后的数据文件。
INPUT_DATA = '../../datasets/flower_photos/flower_processed_data.npy' # 使用1/4数据集处理后的npy文件
# 保存本次训练的模型的路径。
TRAIN_FILE = './mymodel/'
...
STEPS = 100    # 在MacBook Pro 16G上训练100步大概需要1小时, 测试准确率为80.7%, 还有增长空间, 可调成200
BATCH = 32
N_CLASSES = 5

# 不需要从谷歌训练好的模型中加载的参数：
CHECKPOINT_EXCLUDE_SCOPES = 'InceptionV3/Logits,InceptionV3/AuxLogits'
# 需要训练的网络层参数，在fine-tuning的过程中就是最后的全联接层。
TRAINABLE_SCOPES='InceptionV3/Logits,InceptionV3/AuxLogit'


# 获取所有需要从Google训练好的型型加载的参数
...
def main():
    ...
    with tf.Session() as sess:
        # 初始化没有加载进来的变量。注意这个过程一定要在在模型加载之前,否则初始化过程会将加载好的变量重新赋值
