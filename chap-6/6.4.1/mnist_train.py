...
def train(mnist):
    ...
    x = tf.placeholder(tf.float32,
                       [BATCH_SIZE,  # 第一维为batch的大小
                        forward.IMAGE_SIZE,  # 第二三维表示图片尺寸
                        forward.IMAGE_SIZE,
                        forward.NUM_CHANNELS],  # 深度，若是RGB图片则深度为3
                        name='x-input')
    y_ = tf.placeholder(tf.float32, [BATCH_SIZE, forward.OUTPUT_NODE], name='y-input')

    regularizer = tf.contrib.layers.l2_regularizer(REGULARIZATION_RATE)
    y = forward.inference(x, train=True, regularizer=regularizer) # 使用forward中定义的前向传播函数
